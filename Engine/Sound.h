...
public:
  ...
  {
  public:
    FileException(const wchar_t* file,
                  unsigned int line,
...
public:
  ...
  public:
    ...
    ~Channel();
    void
    PlaySoundBuffer(class Sound& s, float freqMod, float vol);
    void
    Stop();

  private:
    void
    RetargetSound(const Sound* pOld, Sound* pNew);
...
public:
  ...
  Get();
  static void
  SetMasterVolume(float vol = 1.0f);
...
public:
  ...
  Sound(Sound&& donor);
  Sound&
  operator=(Sound&& donor);
  void
  Play(float freqMod = 1.0f, float vol = 1.0f);
  void
  StopOne();
  void
  StopAll();
...
private:
  static Sound
  LoadNonWav(const std::wstring& fileName,
             LoopType loopType,
