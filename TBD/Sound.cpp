...
#define CHILI_SOUND_API_EXCEPTION(hr, note)                                    \
  SoundSystem::APIException(hr, _CRT_WIDE(__FILE__), __LINE__, note)

#define CHILI_SOUND_FILE_EXCEPTION(filename, note)                             \
  SoundSystem::FileException(_CRT_WIDE(__FILE__), __LINE__, note, filename)
...
void
SoundSystem::SetMasterVolume(float vol)
{
  HRESULT hr;
...
const WAVEFORMATEX&
SoundSystem::GetFormat()
{
  return *Get().format;
}

void
SoundSystem::PlaySoundBuffer(Sound& s, float freqMod, float vol)
{
        ...
        case LoadType::Local:
          throw CHILI_SOUND_API_EXCEPTION(
              ...
              std::wstring(
                  ...
                  L"A) [ ") +
                  std::wstring(GetDllPath(LoadType::Folder)) +
                  std::wstring(
                      L" ] exist in the same folder as this executable;\n"
                      L"B) [ ") +
                  std::wstring(GetDllPath(LoadType::Local)) +
                  std::wstring(
                      L" ] exist in the same folder as this executable; or\n"
                      L"C) [ XAudio2_7.dll ] be installed on this system via "
                      L"the DirectX"
                      L" Redistributable Installer Version June 2010\n"));
        default:
          assert(
              false &&
              "Bad LoadType encountered in XAudio Dll loading sequence loop");
      }
    }
  }
}
...
SoundSystem::XAudioDll::operator HMODULE() const { return hModule; }

const wchar_t*
SoundSystem::XAudioDll::GetDllPath(LoadType type)
{
  switch (type)
  {
    case LoadType::System:
  ...
  : format(std::make_unique<WAVEFORMATEX>())
{
  ...
  static_assert(
      nChannelsPerSound > 0u,
      "WAVE File Format Error: At least one channel required per audio stream");
  static_assert(nChannelsPerSound <= XAUDIO2_MAX_AUDIO_CHANNELS,
                "WAVE File Format Error: Maximum audio channels per audio "
                "stream exceeded");
  static_assert(
  ...
  static_assert(
      nBitsPerSample > 0u,
      "WAVE File Format Error: Bit depth of 0 bits per sample is not allowed");
  static_assert(nBitsPerSample % 8u == 0,
                "WAVE File Format Error: Bit depth must be multiple of 8");
  ...
  HRESULT hr;
  if (FAILED(hr = DllGetClassObject(__uuidof(XAudio2),
                                    IID_IClassFactory,
  ...
  if (FAILED(hr = pClassFactory->CreateInstance(
                 nullptr, __uuidof(IXAudio2), &pEngine)))
  {
...
void
SoundSystem::DeactivateChannel(Channel& channel)
{
  std::lock_guard<std::mutex> lock(mutex);
  auto i =
      std::find_if(activeChannelPtrs.begin(),
                   activeChannelPtrs.end(),
  ...
  : xaBuffer(std::make_unique<XAUDIO2_BUFFER>())
{
  ...
  public:
    ...
    OnStreamEnd() override
    {
    }
    void STDMETHODCALLTYPE
    OnVoiceProcessingPassEnd() override
    {
    }
    void STDMETHODCALLTYPE
    ...
    void STDMETHODCALLTYPE
    OnBufferEnd(void* pBufferContext) override
    {
      Channel& chan = *reinterpret_cast<Channel*>(pBufferContext);
      chan.Stop();
      {
        std::lock_guard<std::mutex> lock(chan.pSound->mutex);
        chan.pSound->activeChannelPtrs.erase(
            std::find(chan.pSound->activeChannelPtrs.begin(),
                      chan.pSound->activeChannelPtrs.end(),
                      &chan));
    ...
    void STDMETHODCALLTYPE
    OnBufferStart(void* pBufferContext) override
    {
    }
    void STDMETHODCALLTYPE
    OnLoopEnd(void* pBufferContext) override
    {
    }
    void STDMETHODCALLTYPE
    OnVoiceError(void* pBufferContext, HRESULT Error) override
    {
    }
  };
  ...
  HRESULT hr;
  if (FAILED(hr = sys.pEngine->CreateSourceVoice(
                 &pSource, sys.format.get(), 0u, 2.0f, &vcb)))
  {
...
void
SoundSystem::Channel::PlaySoundBuffer(Sound& s, float freqMod, float vol)
{
  ...
  {
    throw CHILI_SOUND_API_EXCEPTION(
        hr, L"Starting playback - submitting source buffer");
  }
  if (FAILED(hr = pSource->SetFrequencyRatio(freqMod)))
  {
    throw CHILI_SOUND_API_EXCEPTION(hr,
                                    L"Starting playback - setting frequency");
  }
...
void
SoundSystem::Channel::RetargetSound(const Sound* pOld, Sound* pNew)
{
  assert(pOld == pSound);
  pSound = pNew;
}
...
Sound::Sound(const std::wstring& fileName, bool loopingWithAutoCueDetect)
  : Sound(fileName,
          loopingWithAutoCueDetect ? LoopType::AutoEmbeddedCuePoints
                                   : LoopType::NotLooping)
{
}
...
Sound::Sound(const std::wstring& fileName, LoopType loopType)
{
  ...
  {
    *this = Sound(
        fileName, loopType, nullSample, nullSample, nullSeconds, nullSeconds);
  }
  else
  {
    *this = LoadNonWav(
...
Sound
Sound::LoadNonWav(const std::wstring& fileName,
                  LoopType loopType,
                  ...
                  float loopEndSeconds)
{
  ...
  if (FAILED(hr = MFCreateSourceReaderFromURL(
                 fileName.c_str(), nullptr, &pReader)))
  {
    throw CHILI_SOUND_API_EXCEPTION(
        hr, L"Creating MF Source Reader\nFilename: " + fileName);
  }
  ...
  wrl::ComPtr<IMFMediaType> pUncompressedAudioType;
  {
    ...
    {
      throw CHILI_SOUND_API_EXCEPTION(hr,
                                      L"setting partial media type major guid");
    }

    if (FAILED(hr = pPartialType->SetGUID(MF_MT_SUBTYPE, MFAudioFormat_PCM)))
    {
      throw CHILI_SOUND_API_EXCEPTION(hr,
                                      L"setting partial media type sub guid");
    }
    ...
    {
      throw CHILI_SOUND_API_EXCEPTION(hr,
                                      L"setting partial type on source reader");
    }
    ...
    {
      throw CHILI_SOUND_API_EXCEPTION(hr,
                                      L"getting complete uncompressed format");
    }
    ...
    {
      throw CHILI_SOUND_API_EXCEPTION(
          hr, L"making sure stream is selected (who the fuck knows?)");
    }
  }
  ...
  // and verifying that format matches sound system channels
  {
    ...
    const auto pFormat = [&]()
    {
      ...
      if (FAILED(hr = MFCreateWaveFormatExFromMFMediaType(
                     pUncompressedAudioType.Get(), &pFormat, &cbFormat)))
      {
        throw CHILI_SOUND_API_EXCEPTION(
            hr, L"loading format info into wave format structure");
      }
      return std::unique_ptr<WAVEFORMATEX, decltype(&CoTaskMemFree)>(
          pFormat, CoTaskMemFree);
    }();
    ...
    // compare format with sound system format
    {
      ...
      {
        throw CHILI_SOUND_FILE_EXCEPTION(
            fileName, L"bad decompressed wave format (nChannels)");
      }
      else if (pFormat->wBitsPerSample != sysFormat.wBitsPerSample)
      {
        throw CHILI_SOUND_FILE_EXCEPTION(
            fileName, L"bad decompressed wave format (wBitsPerSample)");
      }
      else if (pFormat->nSamplesPerSec != sysFormat.nSamplesPerSec)
      {
        throw CHILI_SOUND_FILE_EXCEPTION(
            fileName, L"bad decompressed wave format (nSamplesPerSec)");
      }
      else if (pFormat->wFormatTag != sysFormat.wFormatTag)
      {
        throw CHILI_SOUND_FILE_EXCEPTION(
            fileName, L"bad decompressed wave format (wFormatTag)");
      }
      else if (pFormat->nBlockAlign != sysFormat.nBlockAlign)
      {
        throw CHILI_SOUND_FILE_EXCEPTION(
            fileName, L"bad decompressed wave format (nBlockAlign)");
      }
      else if (pFormat->nAvgBytesPerSec != sysFormat.nAvgBytesPerSec)
      {
        throw CHILI_SOUND_FILE_EXCEPTION(
            fileName, L"bad decompressed wave format (nAvgBytesPerSec)");
      }
    }

    {
      ...
      {
        ~AutoPropVariant() { PropVariantClear(this); }
      } var;

      // get duration attribute (as prop variant) from reader
      if (FAILED(hr = pReader->GetPresentationAttribute(
                     MF_SOURCE_READER_MEDIASOURCE, MF_PD_DURATION, &var)))
      {
        throw CHILI_SOUND_API_EXCEPTION(
            hr, L"getting duration attribute from reader");
      }
      ...
      {
        throw CHILI_SOUND_API_EXCEPTION(
            hr, L"getting int64 out of variant property (duration)");
      }
  ...
  while (true)
  {
    ...
    if (dwFlags & MF_SOURCE_READERF_CURRENTMEDIATYPECHANGED)
    {
      throw std::runtime_error(
          "Type change - not supported by WAVE file format.@ ReadSample");
    }
    ...
    case LoopType::ManualFloat:
      {
        ...
        const unsigned int nFramesPerSec =
            sysFormat.nAvgBytesPerSec / sysFormat.nBlockAlign;
    ...
    case LoopType::AutoFullSound:
      {
        ...
        const unsigned int nFrames =
            sound.nBytes / SoundSystem::GetFormat().nBlockAlign;
      ...
      case LoopType::AutoFullSound:
        {
          ...
          const unsigned int nFrames =
              nBytes / SoundSystem::GetFormat().nBlockAlign;
          assert(nFrames != 0u &&
                 "Cannot auto full-loop on zero-length sound!");
          loopStart = 0u;
...
void
Sound::Play(float freqMod, float vol)
{
...
SoundSystem::APIException::APIException(HRESULT hr,
                                        const wchar_t* file,
                                        unsigned int line,
...
std::wstring
SoundSystem::APIException::GetFullMessage() const
{
  return L"Error Name: " + GetErrorName() + L"\n\n" + L"Error Description: " +
         GetErrorDescription() + L"\n\n" + L"Note: " + GetNote() + L"\n\n" +
         L"Location: " + GetLocation();
}

std::wstring
SoundSystem::APIException::GetExceptionType() const
{
  return L"Sound System API Exception";
}

std::wstring
SoundSystem::APIException::GetErrorName() const
{
  return DXGetErrorString(hr);
}

std::wstring
SoundSystem::APIException::GetErrorDescription() const
{
...
SoundSystem::FileException::FileException(const wchar_t* file,
                                          unsigned int line,
                                          const std::wstring& note,
...
std::wstring
SoundSystem::FileException::GetFullMessage() const
{
...
std::wstring
SoundSystem::FileException::GetExceptionType() const
{
  return L"Sound System File Exception";
}

SoundSystem::MFInitializer::MFInitializer() { hr = MFStartup(MF_VERSION); }
